...
string sessionFileName = "session1.txt";
string tempFileName = "";
...
*/
char randChar(){
  return alphabets[rand()%(sizeof(alphabets)-1)];
}
        ...
        if (userType != "AA"){
          if (maxAddBalanceThisSession > 0.0){
            if (add.addCredit(userType, maxAddBalanceThisSession) == true){
              ...
              uploadUpdates(appendLine,"files/sessions/" + tempFileName);
...
int main(int argc, const char * argv[]) {
  ...
  string line = "", fromTempFile = "";

  ifstream ifile("files/sessions/" + sessionFileName);
  while (ifile.is_open()){
    sessionFileName = "session" + to_string(fileIndex) + ".txt";
    ifstream newfile("files/sessions/" + sessionFileName);
    if (newfile.is_open()){
      fileIndex++;
    } else {
      break;
  ...
  ifile.open("files/sessions/" + tempFileName);
  while(getline(ifile,line)){
    fromTempFile += line + "\n";
  }
  ifile.close();
  cout << fromTempFile << endl;

  ofstream ofile ("files/sessions/" + sessionFileName);
  ofile << fromTempFile;
  ofile.close();

  string fileDelete = "files/sessions/" + tempFileName;
  int n = fileDelete.length();
  char char_array[n+1];
  strcpy(char_array, fileDelete.c_str());
  remove(char_array);
return 0;
}
